...
const TARGET_PACKAGE_ROOT = process.env.OPENCLAW_MIGRATION_TARGET_PACKAGE?.trim() || "/tmp/agentos-openclaw-2026.9.1-runtime/package";
...
const OUTPUT_PATH = process.env.OPENCLAW_MIGRATION_E2E_OUTPUT?.trim() || "docs/evidence/openclaw-2026.6.11-to-2026.9.1-migration.json";
...
async function main() {
  ...
  if (targetIdentity.version !== OPENCLAW_PHASE_2B_TARGET_VERSION || targetIdentity.sourceCommit !== OPENCLAW_PHASE_2B_TARGET_COMMIT) {
    throw new Error("The configured target package is not the exact OpenClaw 2026.9.1 target.");
  }
