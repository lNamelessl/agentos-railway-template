...
- Agent create/update/delete are Gateway-first when the Gateway advertises the lifecycle methods. Native `agents.create` receives only the 2026.9.4 fields it accepts (`name`, `workspace`, `model`, `emoji`, and `avatar`); AgentOS keeps its product-owned id/path, identity, policy skill, worker-profile sidecar, and workspace-manifest side effects outside that native payload.
