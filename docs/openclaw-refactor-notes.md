...
- Direct `runOpenClawJson` also remains in `lib/openclaw/domains/agent-config.ts`, `lib/openclaw/application/channel-directory-service.ts`, `lib/openclaw/surface-adapters.ts`, and the legacy planner runtime path in `lib/openclaw/planner.ts`. The channel directory service is the documented, account-aware structured CLI fallback for OpenClaw 2026.9.4; the other entries are existing config/discovery/planner readers for OpenClaw state or legacy runtime execution paths that have not yet been given stable Gateway RPC equivalents. Telegram log scraping was removed from the normal directory path.
