...
Channels update: 2026-09-15. Telegram route discovery now uses the
account-aware `lib/openclaw/application/channel-directory-service.ts` boundary:
the official structured `openclaw directory ... --json` CLI is the current
transport, with account-scoped OpenClaw config used only when Telegram
directory support is unavailable. Telegram and Discord log scraping are no
longer part of discovery. Discord hierarchy uses the same structured directory
boundary, with account-scoped `channels.discord.guilds` config as a read-only
fallback when the directory command is unsupported.
...
| Telegram directory discovery | `lib/openclaw/application/channel-directory-service.ts` | `openclaw directory groups list --channel telegram --account <id> --json`; config fallback for unsupported directory | Account-aware structured groups, explicit source/status, and config-backed fallback without treating malformed/unsupported results as empty success. | Reads official structured CLI output; config fallback reads `channels.telegram` through the adapter. | No documented `directory.*` Gateway RPC in OpenClaw 2026.9.4; `channels.status` remains lifecycle/status authority | Yes, CLI contract; fallback explicit | Medium | Yes | Yes, until a native directory RPC exists | Replacing directory output with status would remove route discovery. | Directory normalization, account-scope, malformed/unsupported, and Telegram topic tests. |
| Discord discovery | `lib/openclaw/application/channel-directory-service.ts`, `lib/openclaw/domains/channels.ts` | `openclaw directory groups list --channel discord --account <id> --json`; structured `channels.discord.guilds` fallback | Discovers account-scoped guild/channel/role hierarchy through the canonical directory boundary. | Reads structured directory/config only; no log scraping or log-derived route inventory. | `channels.status` remains account lifecycle/status authority; no confirmed directory Gateway RPC | Partial | Medium | Yes | Yes, until a native directory RPC exists | Replacing directory output with status would remove route hierarchy. | Directory normalization, hierarchy, account-scope, malformed/unsupported, and binding tests. |
...
| Routing sync | `lib/openclaw/application/channel-service.ts`, `lib/openclaw/application/channel-route-binding-service.ts` | Native Channel Center binding/policy mutations; explicit audited surface-reconcile bridge for legacy records | Canonical route changes use account-scoped OpenClaw native services. Workspace registry CRUD is metadata-only; legacy repair is previewed and explicit. | Native config, plus narrowly scoped legacy session-store/coordination compatibility effects when explicitly requested. | `config.get/set` and native binding/policy paths | Partial | Medium | Native route/policy paths migrated; legacy repair retained explicitly | Yes for directory CLI and compatibility side effects | Treating workspace metadata as runtime authority would rewrite user routing. | Route binding/policy atomicity, workspace metadata isolation, and repair preview tests. |
