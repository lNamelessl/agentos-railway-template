...
- **Green within seconds of container start** — the `/_agentos/healthz` liveness probe (OpenClaw Gateway + control-plane, 300s timeout), `ON_FAILURE` restart policy, single replica. The app's deep `/api/health` endpoint stays available to operators.
